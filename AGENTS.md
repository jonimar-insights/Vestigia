...
- `auth.ts` Google provider now requests `gmail.send` scope + `access_type=offline&prompt=consent`; Google OAuth tokens persisted to `users` (`gmail_access_token`, `gmail_refresh_token`, `gmail_token_expires_at`) via jwt-callback upsert
